...
def fix_doi_periods(text):
    ...
    # Pattern to match complete DOIs with a trailing period
    # DOIs have format: 10.XXXX/something
    # We want to match the whole DOI and then a period at the end
    pattern = r'(https?://doi\.org/10\.[0-9]+/[^\s\"\'\)}\]<>]+)\.'

    def replacement(match):
        original = match.group(0)
        fixed = match.group(1)
        # Only add to changes if we actually removed a period
        if original != fixed:
